...
import AsistenModal from '../components/AsistenModal';
    ...
    try {
      ...
      const combinedLogs = [...recentSavings, ...recentExpenses]
        ...
        .map(item => {
          ...
          return item.type === 'nabung' ? `Nabung Rp ${rp} (${item.desc})` : `Keluar Rp ${rp} (${item.desc})`;
        });
      ...
      const topTaskRes = db.getFirstSync(`
        ...
        ORDER BY CASE priority WHEN 'mendesak' THEN 1 WHEN 'normal' THEN 2 ELSE 3 END, id ASC LIMIT 1
      `);
      setTopTask(topTaskRes);
      setKonfirmasiTask(false);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        <View style={{ marginHorizontal: 24, borderRadius: 32, elevation: 15, shadowColor: '#000', shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.25, shadowRadius: 20, marginBottom: 15 }}>
          <LinearGradient colors={['#111827', '#1f2937', '#111827']} style={styles.cardKas}>
            ...
            <Text style={styles.totalNominal}>Rp {new Intl.NumberFormat('id-ID').format(totalKas)}</Text>
            ...
            <View style={styles.progressBox}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                  ...
                  <TouchableOpacity onPress={() => { setInputNamaTarget(targetName); setInputNominalTarget(new Intl.NumberFormat('id-ID').format(targetAmount)); setEditTargetMode(true); }} style={styles.btnEdit}>
        ...
        <View style={styles.terminalContainer}>
          ...
          {logs.length === 0 ? (
            ...
            logs.map((log, i) => (
              <Text key={i} style={styles.terminalText}><Text style={{color: '#10b981', fontWeight: 'bold'}}>{'>'} </Text>{log}</Text>
            ))
          )}
        </View>
        ...
        <View style={styles.sectionMargin}>
          <View style={styles.sectionHeaderBox}>
            ...
            <TouchableOpacity onPress={() => setDaftarTugasVisible(true)}>
              <Text style={styles.lihatSemua}>Lihat Semua <FontAwesome5 name="chevron-right" size={8}/></Text>
            </TouchableOpacity>
          </View>
            ...
            <View style={styles.radarCardWrapper}>
              {konfirmasiTask && (
                <View style={styles.konfirmasiOverlay}>
                  <Text style={styles.konfirmasiText}>Yakin kelar bro? 🤔</Text>
                  <View style={styles.konfirmasiBtnRow}>
                    ...
                    <TouchableOpacity onPress={() => selesaiTugas(topTask.id)} style={styles.btnYakin}><Text style={styles.btnYakinText}>Sikat!</Text></TouchableOpacity>
                  </View>
                </View>
              )}
              ...
              <View style={styles.radarCard}>
                ...
                <TouchableOpacity onPress={() => setKonfirmasiTask(true)} style={styles.btnCheck}>
                  <FontAwesome5 name="check" size={12} color="#f87171" />
                </TouchableOpacity>
                <TouchableOpacity style={{ flex: 1, paddingVertical: 8, paddingRight: 8 }} onPress={() => setPreviewData({ visible: true, title: topTask.title, text: topTask.detail || 'Tidak ada detail.', date: topTask.due_date, color: '#fef2f2', icon: 'bolt' })}>
                  <Text style={styles.radarTitle} numberOfLines={1}>{topTask.title}</Text>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 6 }}>
                    <View style={styles.badgePriority}>
                      <FontAwesome5 name="fire" size={10} color="#fff" style={{marginRight: 4}} />
                      <Text style={styles.badgePriorityText}>{topTask.priority}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                      <FontAwesome5 name="clock" size={10} color="#ef4444" style={{marginRight: 4}} />
                      <Text style={styles.radarDate}>{topTask.due_date === 'hari_ini' ? 'Hari Ini' : topTask.due_date === 'besok' ? 'Besok' : topTask.due_date}</Text>
                    </View>
                  </View>
                </TouchableOpacity>
              </View>
          ...
          {notes.length === 0 ? (
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingRight: 24 }}>
              {notes.map((note, index) => {
                ...
                return (
                  <TouchableOpacity key={note.id} style={[styles.noteCard, { backgroundColor: theme.bg, borderColor: theme.border }]} onPress={() => setPreviewData({ visible: true, title: note.title, text: note.content, date: new Date(note.created_at).toLocaleString('id-ID', {day:'numeric', month:'short', hour:'2-digit', minute:'2-digit'}), color: theme.bg, icon: cleanIcon(note.icon) })}>
                    <TouchableOpacity onPress={() => hapusCatatan(note.id)} style={styles.btnHapusCatatan}><FontAwesome5 name="times" size={10} color="#9ca3af" /></TouchableOpacity>
                    <FontAwesome5 name={cleanIcon(note.icon)} size={20} color={theme.text} solid style={{ marginBottom: 12, marginTop: 4 }} />
      ...
      <View style={styles.bottomNav}>
        ...
        <TouchableOpacity style={styles.navItem} onPress={() => setDompetVisible(true)}>
          <FontAwesome5 name="wallet" size={20} color="#9ca3af" /><Text style={styles.navText}>DOMPET</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity style={styles.navItem} onPress={() => setPortalVisible(true)}>
          <FontAwesome5 name="rocket" size={20} color="#9ca3af" /><Text style={styles.navText}>PORTAL</Text>
        </TouchableOpacity>
        
        <TouchableOpacity style={styles.navItem} onPress={() => setAsistenVisible(true)}>
          <FontAwesome5 name="robot" size={20} color="#9ca3af" />
          <Text style={styles.navText}>ASISTEN</Text>
        </TouchableOpacity>
      </View>

      {/* === SEMUA MODAL === */}
      <AddMenuModal visible={modalVisible} onClose={() => setModalVisible(false)} onSuccess={loadDataDariDatabase} />
      <AsistenModal visible={asistenVisible} onClose={() => setAsistenVisible(false)} />
      ...
      <Modal visible={editTargetMode} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          ...
          <View style={styles.editTargetBox}>
            ...
            <TextInput value={inputNamaTarget} onChangeText={setInputNamaTarget} placeholder="Misal: Liburan" style={styles.editTargetInput} />
        ...
        <View style={styles.modalOverlay}>
          ...
          <View style={styles.previewBox}>
            <View style={[styles.previewHeader, { backgroundColor: previewData.color }]}>
              <View style={[styles.previewIconBox, { backgroundColor: '#fff' }]}><FontAwesome5 name={cleanIcon(previewData.icon)} size={18} color="#111827" solid/></View>
              ...
              <TouchableOpacity onPress={() => setPreviewData({ ...previewData, visible: false })} style={styles.btnPreviewClose}><FontAwesome5 name="times" size={14} color="#6b7280" /></TouchableOpacity>
            </View>
...
const styles = StyleSheet.create({
  ...
  btnCheck: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#fff', borderWidth: 2, borderColor: '#fecaca', alignItems: 'center', justifyContent: 'center', marginRight: 15, marginLeft: 5, elevation: 1 },
  radarTitle: { fontSize: 15, fontWeight: '900', color: '#7f1d1d' },
  badgePriority: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#ef4444', paddingHorizontal: 6, paddingVertical: 3, borderRadius: 6 },
  badgePriorityText: { fontSize: 9, fontWeight: '900', color: '#fff', textTransform: 'uppercase', letterSpacing: 1 },
  radarDate: { fontSize: 11, fontWeight: 'bold', color: '#ef4444' },
